...
static uint32_t TouchDeviceGetResource(struct tpd_priv *priv, const struct DeviceResourceNode *resourceNode)
{
    ...
    if (res->GetUint16(resourceNode, "i2c_id", &priv->i2c_client.id, 0) != HDF_SUCCESS) {
        HDF_LOGE("%s: failed to get i2c_id", __func__);
        return HDF_FAILURE;
    }
    if (res->GetUint16(resourceNode, "i2c_addr", &priv->i2c_client.addr, 0) != HDF_SUCCESS) {
...
static struct HdfDriverEntry g_FTSTouchDriverEntry = {
    .moduleVersion = 1,
...
HDF_INIT(g_FTSTouchDriverEntry);