...
static struct DsiCmdDesc g_OnCmd[] = {
    ...
    {0x39, 0, sizeof(g_payLoad4), g_payLoad4},
    {0x39, 0, sizeof(g_payLoad5), g_payLoad5},
    {0x05, 240, sizeof(g_payLoad6), g_payLoad6},
    {0x05, 150, sizeof(g_payLoad7), g_payLoad7},
};
...
static uint32_t PanelGetResource(struct PanelDevice *priv, const struct DeviceResourceNode *resourceNode)
{
    ...
    HDF_LOGD("%s: rst_pin=%d, te_pin=%d", __func__, priv->pin_rst.pin, priv->pin_te.pin);
    return HDF_SUCCESS;
}
...
static struct HdfDriverEntry g_RM69330DriverEntry = {
    .moduleVersion = 1,
